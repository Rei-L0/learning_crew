...
async def upload_and_analyze(
    plan_file: UploadFile = File(...), report_file: UploadFile = File(...)
):
    """
    ...
    """
    logger.info(
        f"파일 수신: [계획서] {plan_file.filename}, [결과보고서] {report_file.filename}"
    )
    ...
    try:
        ...
        return {"filename": report_file.filename, "analysis_result": api_response}
    except Exception as e:
...
def run_local_analysis():
    ...
    try:
        ...
        # 4. JSON 형식으로 결과 반환
        return {"file_processed": file_source, "analysis_result": api_response}
    except Exception as e:
        logger.error(f"API 처리 중 오류: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
if __name__ == "__main__":
    ...
    # uvicorn.run()에 "모듈이름:앱이름"을 문자열로 전달합니다.
